...
class Splash extends FlxState
{
        ...
        override function update(elapsed:Float)
        {
                if (logo != null)
                {
                        ...
                        // ADICIONADO: Suporte a Toque (FlxG.touches)
                        var pressedTouch:Bool = false;
                        #if mobile
                        for (touch in FlxG.touches.list) {
                            if (touch.justPressed) pressedTouch = true;
                        }
                        #end

                        if (FlxG.keys.justPressed.SPACE || FlxG.keys.justPressed.ENTER || pressedTouch)
                        {
                                finish();
                        }
                }
        ...
        function logoFunc()
        {
                // Ajuste para evitar crash no Android ao ler diretório de assets diretamente
                var imgName:String = 'logo'; // Fallback caso o sorteio falhe
                
                #if sys
                try {
                    var folder = FileSystem.readDirectory(Paths.getPrimaryPath('images/branding'));
                    if (folder.length > 0) {
                        var img = folder[FlxG.random.int(0, folder.length - 1)];
                        imgName = img.replace('.png', '');
                    }
                } catch(e) {
                    trace("Erro ao ler branding: " + e.message);
                }
                #end

                logo = new FlxSprite().loadGraphic(Paths.image('branding/$imgName'));
                logo.screenCenter();
                logo.visible = false;
                add(logo);
